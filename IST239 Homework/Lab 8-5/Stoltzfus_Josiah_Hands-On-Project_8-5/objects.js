"use strict";
/*
Josiah Stoltzfus, July 12, 2025, IST239-W01, Hands-On Project 8-5

In Hands-On Project 8-5, I learned that you need to use "this" to correctly assign properties in a constructor function. I fixed the constructor function for the chess piece so it actually stores the data using "this". I also corrected the object creation 
and fixed a typo in the function name.
*/

// Constructor Function for Chess Piece
function piece(color, rank) {
   // FIXED: Changed 'color' to 'this.color'
   this.color = color;    // the color of the piece (white or black)
   // FIXED: Changed 'rank' to 'this.rank'
   this.rank = rank;      // the piece rank (Pawn, Knight, Bishop, Rook, Queen, King)
   // FIXED: Changed 'square' to 'this.square'
   this.square = null;    // the square the piece occupies
   // FIXED: Changed 'image' to 'this.image'
   ...
   for (let i = 0; i < 8; i++) {         // loop through the chess board rows
      for (let j = 0; j < 8; j++) {      // loop through the chess board columns
         if (game.board[i][j].length === 2) {
            ...
            // FIXED: Changed 'Piece' to 'piece' because the function name should be lowercase
