import {STASH_LOCATIONS} from "../../../data/Stash-locations.js";
import ActionToggle from "../../ActionToggle.jsx";
import SelectionInput from "../../SelectionInput.jsx";
import ActionButton from "../../ActionButton.jsx";

export default function StashEditCard({changeLocation, setIsEditing, selectedLocation, setSelectedLocation}) {
    return (
        <div className="stash-card stash-edit-card">
            <p className="stash-card-label">Change Location</p>
            <SelectionInput options={STASH_LOCATIONS} selectedValue={selectedLocation} setSelectedValue={setSelectedLocation}/>
            <div className="action-buttons">
                <ActionButton variant="action-button-confirm" onClick={changeLocation}>Confirm</ActionButton>
                <ActionButton variant="action-button-cancel" onClick={() => setSelectedLocation(null)}>Cancel</ActionButton>
            </div>
            <ActionToggle onClick={() => {
                setIsEditing(false);
            }}/>
        </div>
    )
}